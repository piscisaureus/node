...
WriteStream.prototype.setColor = function(value) {
  if (!this.writable) 
    throw new Error('Stream not writable');
  ...
  // 'white', 'grey', 'default', or any unknown color,
  // these all mean white
  if (!rgb) {
    rgb = [true, true, true];
  }

  binding.setTextColor.apply(global, [this.fd].concat(rgb))
};

WriteStream.prototype.setIntensity = function(value) {
  if (!this.writable) 
    throw new Error('Stream not writable');

  binding.setTextIntensity(this.fd, value === 'bold');
};